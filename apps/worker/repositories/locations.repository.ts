...
export function createLocationRepository(db: Database, organizationId: string) {
  ...
  return {
    ...
    async createBranch(data: Omit<NewBranch, 'id' | 'organizationId' | 'createdAt' | 'updatedAt'>) {
      const now = new Date()
    ...
    async updateBranch(id: string, data: Partial<Omit<NewBranch, 'id' | 'organizationId' | 'createdAt' | 'updatedAt'>>) {
    ...
    async createLocation(data: Omit<NewLocation, 'id' | 'organizationId' | 'createdAt' | 'updatedAt'>) {
      const now = new Date()
      const row = { ...data, id: generateId(), organizationId, createdAt: now, updatedAt: now }
    ...
    async updateLocation(id: string, data: Partial<Omit<NewLocation, 'id' | 'organizationId' | 'createdAt' | 'updatedAt'>>) {
